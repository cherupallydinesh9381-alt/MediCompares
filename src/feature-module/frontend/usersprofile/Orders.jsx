...
const Orders = ({ HomeNavigate, serviceType }) => {
  ...
  const downloadInvoice = async (orderId = "Order", vendorName = "") => {
    try {
      const element = invoiceRef.current;
      if (!element) {
        toast.dismiss();
        toast.error("Invoice template element not ready.");
        return;
      }
      ...
      const sanitizedVendor = vendorName ? `_${vendorName.replace(/[^a-z0-9]/gi, "_")}` : "";
      pdf.save(`Invoice_${orderId}${sanitizedVendor}.pdf`);
      toast.dismiss();
    } catch (error) {
      toast.dismiss();
  ...
  const handleDownloadVendorInvoice = (order, vendor) => {
    const displayName = vendor ? vendor.name : "Combined";
    toast.loading(`Generating invoice for ${displayName}...`);
    setSelectedInvoiceOrder(order);
    setSelectedInvoiceVendor(vendor);
    setTimeout(() => {
      downloadInvoice(order.orderId, vendor ? vendor.name : "Combined");
    }, 400);
  };

  const handleDownloadAllVendorInvoices = async (order) => {
    const vendors = getOrderVendors(order);
    if (vendors.length === 0) {
      toast.error("No vendors found for this order");
      return;
    }
    for (let i = 0; i < vendors.length; i++) {
      handleDownloadVendorInvoice(order, vendors[i]);
      // Wait for a second and a half to prevent browser blocking simultaneous downloads
  ...
  const bSummary = selectedOrder?.billingSummary || {};
  const walletAmountUsed = selectedOrder?.walletAmount ?? selectedOrder?.walletamount ?? bSummary.walletAmount ?? 0;
  const couponDiscountApplied = selectedOrder?.discount ?? selectedOrder?.couponAmount ?? bSummary.couponAmount ?? 0;

  const valWithoutCouponAndWithoutWallet = Number(bSummary.withoutCouponAndWithoutWallet ?? (productSubtotal + deliveryFee));
  const valWithCouponAndWithoutWallet = Number(bSummary.withCouponAndWithoutWallet ?? (productSubtotal + deliveryFee - couponDiscountApplied));
  const valWithoutCouponAndWithWallet = Number(bSummary.withoutCouponAndWithWallet ?? Math.max(0, (productSubtotal + deliveryFee) - walletAmountUsed));
  const valWithCouponAndWithWallet = Number(bSummary.withCouponAndWithWallet ?? grandTotal);
          ...
          <div className="row">
              ...
              >
                ...
                >
                  ...
                  >
                    <input
                      type="text"
                      placeholder="Search by Order ID and Name"
                      value={searchTerm}
                    ...
                    <div key={index} className="order-card">
                      <div className="order-header">
                        ...
                        <div className="d-flex align-items-center gap-2">
                          {!["completed", "delivered", "cancelled", "canceled", "failed"].includes(order.orderStatus?.toLowerCase()) && (
                            <button
                              type="button"
                      ...
                      <div className="row align-items-center">
                          ...
                          <div className="row">
                                ...
                                <>
                                  ...
                                  <div className="info-value">
                                    {order.items[0].productDetails?.vendorDetails?.[0]?.location?.coordinates ? (
                                      <a
                                        href={`https://www.google.com/maps?q=${order.items[0].productDetails.vendorDetails[0].location.coordinates[1]
                                          },${order.items[0].productDetails.vendorDetails[0].location.coordinates[0]
                                          }`}
                                        target="_blank"
                            ...
                            <div className="col-12">
                              {(order?.selectedDate && order?.selectedTimeSlot) && (
                                <>
                                  <div className="info-label">Appointment Date & Time :</div>
                                  <div className="info-value">
                                    {(() => {
                                      try {
                        ...
                        <div className="col-md-4 col-12 payment-box">
                          ...
                          {(order.orderStatus !== 'failed') && (order.orderStatus !== 'cancelled') && (
                            <div className="d-flex flex-wrap justify-content-end gap-2 mt-1">
                              ...
                              {order?.paymentStatus !== 'pending' && order?.paymentStatus !== 'cancelled' && (
                                <div className="dropdown d-inline-block">
                                  <button
                                    type="button"
                                    className="btn order-action-btn dropdown-toggle"
                                  ...
                                  <ul className="dropdown-menu dropdown-menu-end shadow-sm border-0" style={{ borderRadius: "8px", minWidth: "200px", padding: "6px 0" }}>
                                    <li>
                                      <button
                                        type="button"
                                        className="dropdown-item d-flex align-items-center gap-2 py-2 fw-semibold text-primary"
                                        onClick={() => handleDownloadVendorInvoice(order, null)}
                                        style={{ fontSize: "13px" }}
                                      >
                                        <i className="fas fa-file-invoice" style={{ fontSize: "11px" }}></i>
                                        Combined Invoice
                                      </button>
                                    </li>
                                    {getOrderVendors(order).length > 0 && (
                                      <>
                                        <li><hr className="dropdown-divider" style={{ margin: "4px 0", borderColor: "#f0f0f0" }} /></li>
                                        <li className="dropdown-header text-muted" style={{ fontSize: "10px", fontWeight: "bold", textTransform: "uppercase", padding: "4px 16px" }}>Individual Providers</li>
                                        {getOrderVendors(order).map((vendor) => (
                                          <li key={vendor.vendorId || vendor.name}>
                                            <button
                                              type="button"
                                              className="dropdown-item d-flex align-items-center gap-2 py-2"
                                              onClick={() => handleDownloadVendorInvoice(order, vendor)}
                                              style={{ fontSize: "13px", color: "#333", paddingLeft: "24px" }}
                                            >
            ...
            {showModel && (
              ...
              >
                ...
                >
                  {/* HEADER */}
                  <div style={{ background: "linear-gradient(135deg, #8059ca 0%, #6741b8 100%)", padding: "20px 24px 18px" }}>
                      ...
                      <div>
                        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "6px" }}>
                          <div style={{ width: 36, height: 36, borderRadius: "10px", background: "rgba(255,255,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                            <i className="fas fa-box" style={{ color: "#fff", fontSize: "15px" }} />
                          </div>
                          <h5 style={{ margin: 0, fontWeight: 700, fontSize: "17px", color: "#fff" }}>Order Details</h5>
                        </div>
                        <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
                          <span style={{ fontSize: "12px", color: "rgba(255,255,255,0.75)" }}>Order ID:</span>
                  ...
                  <div style={{ overflowY: "auto", flex: 1, padding: "20px 24px" }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginBottom: "20px" }}>
                      {selectedOrder?.items?.length > 0 ? selectedOrder.items.map((orderItem, idx) => {
                        ...
                        return (
                          <div key={idx} style={{ display: "flex", gap: "14px", padding: "14px", background: "#fdfaff", border: "1.5px solid #f1edfa", borderRadius: "14px", alignItems: "center" }}>
                            <div style={{ width: 72, height: 72, borderRadius: "10px", border: "1px solid #e8e3f7", background: "#fff", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, overflow: "hidden" }}>
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              ...
                              <p style={{ margin: "0 0 4px", fontSize: "11.5px", color: "#8059ca", fontWeight: 600 }}>
                                <i className="fas fa-store" style={{ marginRight: "5px", fontSize: "10px" }} />{vendorName}
                              </p>
                            ...
                            <div style={{ textAlign: "right", flexShrink: 0 }}>
                              ...
                              <div style={{ fontWeight: 800, fontSize: "14.5px", color: "#8059ca" }}>₹{itemTotal}</div>
                            </div>
                          </div>
                        );
                      }) : (
                        <div style={{ textAlign: "center", color: "#94a3b8", padding: "24px 0" }}>
                          <i className="fas fa-box-open" style={{ fontSize: "28px", marginBottom: "8px" }} />
                    ...
                    <div style={{ background: "#f8fafc", border: "1px solid #e9ecef", borderRadius: "14px", padding: "16px", display: "flex", flexDirection: "column", gap: "10px" }}>
                      ...
                      {couponDiscountApplied > 0 && (
                        <div style={{ display: "flex", justifyContent: "space-between", fontSize: "13px", color: "#16a34a", background: "#f0fdf4", borderRadius: "8px", padding: "6px 10px" }}>
                          ...
                          <span style={{ fontWeight: 700 }}>−₹{couponDiscountApplied.toFixed(2)}</span>
                        </div>
                      )}
                      {walletAmountUsed > 0 && (
                        <div style={{ display: "flex", justifyContent: "space-between", fontSize: "13px", color: "#059669", background: "#ecfdf5", borderRadius: "8px", padding: "6px 10px" }}>
                          <span style={{ fontWeight: 600 }}>Wallet Deduction</span>
                          <span style={{ fontWeight: 700 }}>−₹{walletAmountUsed.toFixed(2)}</span>
                        </div>
                      )}
                      ...
                      <div style={{ borderTop: "1.5px dashed #ddd6fe", paddingTop: "12px", marginTop: "4px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        ...
                        <span style={{ fontWeight: 800, fontSize: "18px", color: "#8059ca" }}>₹{grandTotal.toFixed(2)}</span>
                      </div>

                      {/* Detailed Breakdown for Clarification */}
                      <div style={{ borderTop: "1px solid #e2e8f0", paddingTop: "10px", marginTop: "8px" }}>
                        <div style={{ fontSize: "11px", fontWeight: 700, color: "#64748b", marginBottom: "8px", textTransform: "uppercase", letterSpacing: "0.5px" }}>Billing Breakdown (Clarification)</div>
                        <div style={{ display: "flex", flexDirection: "column", gap: "6px", background: "#f1f5f9", padding: "10px", borderRadius: "8px", border: "1px solid #e2e8f0" }}>
                          ...
                          <div style={{ display: "flex", justifyContent: "space-between", fontSize: "12px", color: "#8059ca", fontWeight: 600 }}>
